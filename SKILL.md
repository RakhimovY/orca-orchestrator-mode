...
- Exclusive resources (a browser automated via MCP/CDP = ONE browser regardless of Orca tab profiles; emulator; prod DB): keep a lease table in the checkpoint; default = the second task queues until the holder's worker_done. `orca tab profile` isolates only Orca's built-in browser.
