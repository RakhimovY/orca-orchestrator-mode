...
- **worker-mode: STATUS-to-bus drift (the 0.5.0 STATUS-injection RED was only half-fixed).** § Reporting forbids routine STATUS on the org bus, but § Session start ("First STATUS to the bus") and § Phase boundaries ("STATUS to the bus: what closed...") still mandated STATUS unconditionally - a worker reading top-down would emit a protocol-accepted + boundary STATUS on the org bus and inject into the coordinator's live session, the exact 0.5.0 RED. Both sections (and the permission-mismatch report channel) are now bus-type-aware, deferring to § Reporting: file bus = STATUS; org bus = silent (notes in commits + plan file), mismatch via worker_done/escalation. Both editions.
- **worker-mode (private edition only): stale Fable binding (worker-side of the 0.6.1 launch-health RED).** 0.6.1 moved the orchestrator's hard/security tier off the unavailable Fable 5 onto Opus 4.8 1M, but worker-mode's subagent-routing table still sent hard/debug subagents to "frontier (Fable)" - a worker spawning a hard-debug subagent would hit a silent model-init hang. Now mirrors the orchestrator binding (Fable 5 unavailable 2026-06-13 → Opus 4.8 1M; re-check). The portable edition is model-agnostic ([frontier tier]) and unaffected.
