...
- Bus discipline is BUS-TYPE-ADAPTIVE (every message on the orchestration bus injects into the coordinator's live session and can interrupt the user mid-conversation; a file bus is passive - the coordinator polls it):
  ...
  - Both buses: a blocking signal MUST be EMITTED to the bus as a real message - `decision_gate` (need an A/B call to proceed), `ask`/QUESTION (blocking question), `BLOCKED` (cannot continue + reason) - and THEN you wait. Recording the block ONLY in the worktree status/comment or your commits is INVISIBLE to the coordinator: it has nothing to read, and you deadlock silently. The "end your turn cleanly" rule below applies AFTER you have emitted the signal - NEVER end a turn blocked on the coordinator with no bus message in flight. (Live RED 2026-06-13: a worker hit an architecture fork, wrote it to its worktree status, ended its turn expecting the coordinator to "see the decision_gate" - but never emitted one; silent deadlock until the user noticed the stall.)
- Tracked dispatch (taskId/dispatchId present) = exactly one `worker_done` at the end (even on failure), a STRUCTURED payload, not prose: `{summary, files_changed[], verify: pass|fail (+failing output), key_decisions[], follow_ups[], blockers[]}`. The coordinator acts on this payload, not your diff. **`key_decisions[]` = the non-obvious choices YOU made that a dependent task or a reviewer needs** (a signature you defined, status codes, a param you introduced, a library/pattern you picked) - distinct from any predecessor decisions you were handed. Omit it and the coordinator must reconstruct your choices from the diff, breaking handoff to dependent tasks. Do NOT emit a success `worker_done` until your `verify` command is green; a `verify` failure you can fix = fix it (bounded attempts) before escalating, never bounce a first red to the coordinator.
...
- Tracker issue: log key decisions/gotchas to the task AS YOU WORK (a tracker comment is NOT a bus message - it does not inject into the coordinator's session, so write freely) + a final comment "what was done + links (PR/commits)"; never fabricate statuses.
